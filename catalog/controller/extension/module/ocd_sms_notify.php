...
class ControllerExtensionModuleOcdSmsNotify extends Controller {
    ...
    public function order(&$route, &$args) {
        ...
        if ($order_info) {
            ...
            if (!$order_info['order_status_id'] && !$admin_order && !$sendsms) {
                $this->model_extension_module_ocd_sms_notify->sendServiceSms($order_info['order_id']);
            }

            // Send SMS for Order Status
            if ($order_status_id && $admin_order && $sendsms) {
                $this->model_extension_module_ocd_sms_notify->sendOrderStatusSms($order_id, $order_status_id, $comment, $sendsms);
            } elseif ($order_status_id && !$admin_order && $this->config->get('sms_notify_force')) {
                $this->model_extension_module_ocd_sms_notify->sendOrderStatusSms($order_info['order_id'], $order_status_id, $comment, true);
            }
        }
    }
    ...
    public function review(&$route, &$args) {
        ...
        $this->model_extension_module_ocd_sms_notify->sendReviewsSms($product_id);
    }
}