...
class ControllerSettingStore extends Controller {
	...
	protected function getForm() {
		...
		foreach ($languages as $key=>$language) {
			...
			$data['logo_multi'][$language['language_id']] = $data['logo'];
			$data['thumb_multi'][$language['language_id']] = $data['thumb'];
		}

		if (isset($this->request->post['config_logo_multi'])) {
			$data['config_logo_multi'] = $this->request->post['config_logo_multi'];
		} elseif ((array)$this->config->get('config_logo_multi')) {
			$data['config_logo_multi'] = (array)$this->config->get('config_logo_multi');
		} else {
			foreach ($languages as $key=>$language) {
				$data['config_logo_multi'][$language['language_id']] = $data['config_logo'];
			}			
		}

		$no_image = $this->model_tool_image->resize('no_image.png', 100, 100);
		if (isset($this->request->post['config_logo_multi'])) {
			foreach ($this->request->post['config_logo_multi'] as $language_id=>$config_logo_multi) {
				if (is_file(DIR_IMAGE . $config_logo_multi)) {
					$data['logo_multi'][$language_id] = $this->model_tool_image->resize($config_logo_multi, 100, 100);
				} else {
					$data['logo_multi'][$language_id] = $no_image;
				}
			}
		} elseif ((array)$this->config->get('config_logo_multi')) {
			foreach ((array)$this->config->get('config_logo_multi') as $language_id=>$config_logo_multi) {
				if (is_file(DIR_IMAGE . $config_logo_multi)) {
					$data['logo_multi'][$language_id] = $this->model_tool_image->resize($config_logo_multi, 100, 100);
				} else {
					$data['logo_multi'][$language_id] = $no_image;
		...
		if (isset($this->request->post['config_image_multi'])) {
			$data['config_image_multi'] = $this->request->post['config_image_multi'];
		} elseif ((array)$this->config->get('config_image_multi')) {
			$data['config_image_multi'] = (array)$this->config->get('config_image_multi');
		} else {
			foreach ($languages as $key=>$language) {
				$data['config_image_multi'][$language['language_id']] = $data['config_image'];
			}			
		}
		
		if (isset($this->request->post['config_image_multi'])) {
			foreach ($this->request->post['config_image_multi'] as $language_id=>$config_image_multi) {
				if (is_file(DIR_IMAGE . $config_image_multi)) {
					$data['thumb_multi'][$language_id] = $this->model_tool_image->resize($config_image_multi, 100, 100);
				} else {
					$data['thumb_multi'][$language_id] = $no_image;
				}
			}
		} elseif ((array)$this->config->get('config_image_multi')) {
			foreach ((array)$this->config->get('config_image_multi') as $language_id=>$config_image_multi) {
				if (is_file(DIR_IMAGE . $config_image_multi)) {
					$data['thumb_multi'][$language_id] = $this->model_tool_image->resize($config_image_multi, 100, 100);
				} else {
					$data['thumb_multi'][$language_id] = $no_image;
		...
		if (isset($this->request->post['config_name_multi'])) {
			$data['config_name_multi'] = $this->request->post['config_name_multi'];
		} elseif ((array)$this->config->get('config_name_multi')) {
			$data['config_name_multi'] = (array)$this->config->get('config_name_multi');
		} else {
			foreach ($languages as $key=>$language) {
				$data['config_name_multi'][$language['language_id']] = $data['config_name'];
			}			
		}

		if (isset($this->request->post['config_comment_multi'])) {
			$data['config_comment_multi'] = $this->request->post['config_comment_multi'];
		} elseif ((array)$this->config->get('config_comment_multi')) {
			$data['config_comment_multi'] = (array)$this->config->get('config_comment_multi');
		} else {
			foreach ($languages as $key=>$language) {
				$data['config_comment_multi'][$language['language_id']] = $data['config_comment'];
			}			
		}

		if (isset($this->request->post['config_meta_description_multi'])) {
			$data['config_meta_description_multi'] = $this->request->post['config_meta_description_multi'];
		} elseif ((array)$this->config->get('config_meta_description_multi')) {
			$data['config_meta_description_multi'] = (array)$this->config->get('config_meta_description_multi');
		} else {
			foreach ($languages as $key=>$language) {
				$data['config_meta_description_multi'][$language['language_id']] = $data['config_meta_description'];
			}
		}

		if (isset($this->request->post['config_meta_keyword_multi'])) {
			$data['config_meta_keyword_multi'] = $this->request->post['config_meta_keyword_multi'];
		} elseif ((array)$this->config->get('config_meta_keyword_multi')) {
			$data['config_meta_keyword_multi'] = (array)$this->config->get('config_meta_keyword_multi');
		} else {
			foreach ($languages as $key=>$language) {
				$data['config_meta_keyword_multi'][$language['language_id']] = $data['config_meta_keyword'];
			}
		}

		if (isset($this->request->post['config_meta_title_multi'])) {
			$data['config_meta_title_multi'] = $this->request->post['config_meta_title_multi'];
		} elseif ((array)$this->config->get('config_meta_title_multi')) {
			$data['config_meta_title_multi'] = (array)$this->config->get('config_meta_title_multi');
		} else {
			foreach ($languages as $key=>$language) {
				$data['config_meta_title_multi'][$language['language_id']] = $data['config_meta_title'];
			}
		}

		if (isset($this->request->post['config_address_multi'])) {
			$data['config_address_multi'] = $this->request->post['config_address_multi'];
		} elseif ((array)$this->config->get('config_address_multi')) {
			$data['config_address_multi'] = (array)$this->config->get('config_address_multi');
		} else {
			foreach ($languages as $key=>$language) {
				$data['config_address_multi'][$language['language_id']] = $data['config_address'];
			}
		}

		if (isset($this->request->post['config_open_multi'])) {
			$data['config_open_multi'] = $this->request->post['config_open_multi'];
		} elseif ((array)$this->config->get('config_open_multi')) {
			$data['config_open_multi'] = (array)$this->config->get('config_open_multi');
		} else {
			foreach ($languages as $key=>$language) {
				$data['config_open_multi'][$language['language_id']] = $data['config_open'];
			}
		}

		$this->load->language('setting/setting_counter');
		...
		$data['text_meta_tags']              = $this->language->get('text_meta_tags');
		$data['entry_meta_tags']             = $this->language->get('entry_meta_tags');
		$data['help_entry_meta_tags']        = $this->language->get('help_entry_meta_tags');
		$data['placeholder_entry_meta_tags'] = $this->language->get('placeholder_entry_meta_tags');
		$data['example_meta_tags']           = $this->language->get('example_meta_tags');
		if (isset($this->request->post['config_meta_tags'])) {
			$data['config_meta_tags'] = $this->request->post['config_meta_tags']; 
		} else {
			$data['config_meta_tags'] = $this->config->get('config_meta_tags');
		}
